...
export default function ProgramPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setProgram({ ...program, content: editContent });
      setIsEditing(false);
  ...
  const handleDeleteProgram = async () => {
    ...
    if (program.created_by !== user.id) {
      alert("אתה יכול למחוק רק תכניות שיצרת");
      return;
    }

    if (!confirm("האם אתה בטוח שברצונך למחוק את התכנית הזו? פעולה זו לא ניתנת לביטול.")) {
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center" style={{ fontFamily: 'var(--font-heebo)' }}>
        <div className="text-center">
          ...
          <div className="text-xl mb-4" style={{ color: '#FFFFFF' }}>טוען תכנית...</div>
        </div>
      </div>
    );
  }
  ...
  if (!loading && !program) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center" style={{ fontFamily: 'var(--font-heebo)' }}>
        <div className="text-center">
          <div className="text-xl mb-4" style={{ color: '#FFFFFF' }}>תכנית לא נמצאה</div>
          <Link href="/programs" className="text-blue-400 hover:text-blue-300" style={{ fontFamily: 'var(--font-mono)' }}>
            חזרה לרשימת התכניות
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: 'var(--font-heebo)' }}>
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          <Link href="/programs" className="wireframe-border px-3 py-1 mb-4 inline-block" style={{ color: '#FFFFFF', fontFamily: 'var(--font-mono)' }}>
            ← חזרה לתכניות
          </Link>
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              {program.description && (
                <p className="text-xl mb-4" style={{ color: '#FFFFFF', opacity: 0.7 }}>{program.description}</p>
              )}
                  ...
                  <div className="wireframe-border p-4 bg-transparent">
                    <div className="relative aspect-square w-full max-w-md mx-auto">
                      <Image
                        ...
                        unoptimized={program.image_url.includes('youtube.com') || program.image_url.includes('img.youtube.com')}
                      />
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-6">
                    {Object.entries(contentSections).map(([section, content]) => (
                      <div key={section} className="wireframe-border p-6 bg-transparent">
                        <h2 className="text-2xl font-bold mb-4 glitch-text" style={{ color: '#008C9E', fontFamily: 'var(--font-heebo)' }}>
                          {section}
                        </h2>
